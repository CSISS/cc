...
edu.gmu.csiss.covali.search = {
	getResults: function(name, title, request){
		this.resultDialog(name, title)
		var tableid = '#producttable-' + name;
	...
	resultDialog: function(name, title){
		var dialogName = 'edu.gmu.csiss.covali.search.jsframe.SearchResults.' + name;
	...
	granules: function(name){
		api =  $('#producttable-' + 'main').DataTable();
		...
		this.getResults('granules', "Granules for " + name, request);
...
//			'					    </span>'+
	...
	searchDialogsearchButtonAction: function(){
		...
		this.getResults('main', 'Search Results', request);
	},
